...
serve(async (req) => {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("process_waiting_list_timeouts");
    if (error) throw error;

    console.log("Waiting list timeouts processed:", data);

    return new Response(
      JSON.stringify({ success: true, results: data, timestamp: new Date().toISOString() }),
