-- Attendance V3: read-only monthly analytics for the dashboard plus scan-path indexes.
...
CREATE INDEX IF NOT EXISTS idx_attendance_logs_library_month_student
  ON public.attendance_logs (library_id, date DESC, student_id);

DROP FUNCTION IF EXISTS public.get_monthly_attendance_analytics(UUID, DATE);
