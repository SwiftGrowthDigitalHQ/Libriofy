-- Libriofy Library Access Key security fix
  ...
  SELECT
    ...
    OR EXISTS (
      ...
      WHERE ur.user_id = _user_id
        ...
        AND ur.role IN ('library_owner', 'staff', 'super_admin')
    );
$$;
...
BEGIN
  FOR v_library IN
    ...
    WHERE NOT EXISTS (
      SELECT 1
      FROM public.library_access_keys lak
      WHERE lak.library_id = l.id
    )
  LOOP
...
BEGIN
  ...
  IF auth.uid() IS NOT NULL
    AND NOT public.can_access_library(auth.uid(), p_library_id) THEN
